...
function build_model(relax_x::Bool = false, relax_z::Bool = false)
  ...
  #ReplaceWithinLife = @constraint(m, [i in Components, t in 1:T],sum(x[i, (t,r)] for r in (t+1):min((t+U[i]), T+1)) == sum(x[i, (r,t)] for r in 0:(t-1)))
  ...
  ReplaceEveryComponent = @constraint(m, [i in Components], sum(x[i, (0, t)] for t in 1:(T+1)) == 1)
